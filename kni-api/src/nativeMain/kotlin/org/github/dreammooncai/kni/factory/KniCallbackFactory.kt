...
inline fun <reified R> jobject.asKniCallback(): () -> R = with(kni) {
    val invoke =
        this@asKniCallback.asKniClass.methods.firstOrNull { it.name == "invoke" }
            ?.asKni(this@asKniCallback)?.ref ?: error("无法获取 invoke 方法")
...
inline fun <reified R> jobject.asKniArgsCallback(): (args: List<*>) -> R = with(kni) {
    val invoke =
        this@asKniArgsCallback.asKniClass.methods.firstOrNull { it.name == "invoke" }
            ?.asKni(this@asKniArgsCallback)?.ref ?: error("无法获取 invoke 方法")
    return { args ->
...
inline fun <reified P1, reified P2, reified P3, reified R> jobject.asKniCallback(): (P1?, P2?, P3?) -> R =
    with(kni) {
        val invoke = this@asKniCallback.asKniClass.methods.firstOrNull { it.name == "invoke" }
            ?.asKni(this@asKniCallback)?.ref ?: error("无法获取 invoke 方法")
